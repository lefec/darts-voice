reactive_trigger <- function() {
  ...
  list(
    ...
    trigger = function() {
      counter(shiny::isolate(counter()) + 1)
    }
  )
}
...
leg_1_player <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(player_name, color, total_score = 301,
      ...
      attempt::stop_if_not(player_name, is.character)
      attempt::stop_if_not(player_name, ~length(.) == 1)

      attempt::stop_if_not(total_score, is.numeric)
      attempt::stop_if_not(total_score, ~ . %in% c(301, 501))

      attempt::stop_if_not(double_out, is.logical)
    ...
    print = function(...) {
      cat("Leg:", self$status(), "| Player name:", self$player_name,
          ...
          dplyr::if_else(self$double_out, "Double", "Single"), "out \n", sep = "\t")
    ...
    score = function(value) {
      value_checks <- c(is.numeric(value), dplyr::between(value, 0, 180))
      ...
      if (all(value_checks)) {
        ...
        hist_tmp <-
          tibble::tibble(score = value, darts_count = 3,
                         round = self$round() + 1L,
                         remaining = self$remain() - value,
                         date = Sys.time())

        private$hist <- dplyr::bind_rows(private$hist, hist_tmp)
    ...
    getHist = function() {
      if (private$reactive) private$rx_trigger$depend()
      private$hist %>%
        dplyr::select(round, score, remaining) %>%
    ...
    saveHist = function() {
      ...
      hist_to_save <-
        ...
        dplyr::mutate(game_id = .game_id,
                      leg_id = 1L) %>%
        dplyr::select(game_id, leg_id, dplyr::everything())
...
leg_n_players <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(player_names, total_score = 301, double_out = FALSE,
      ...
      attempt::stop_if_not(player_names, is.character)
      attempt::stop_if_not(player_names, ~ length(.) >= 1)
      ...
      legs <- purrr::map2(player_names, self$player_colors,
                          leg_1_player$new,
    ...
    score = function(value, rest) {
      attempt::stop_if_not(self$has_finished, is.na)
      ...
      if (self$legs[[self$next_score]]$valid_score) {
        ...
        self$next_score <-
          dplyr::if_else(self$next_score == length(self$players),
                         1, self$next_score + 1)
      }
...
# game_n_players -------------------------------------------------------------
game_n_players <- R6::R6Class(
  "game_n_players",
  public = list(

    game_hist    = tibble::tibble(),
    leg          = list(),
    leg_id       = 1,
    ...
    player_won = function(winning_player) {
      ...
      purrr::walk(self$leg$legs, ~ .x$saveHist())

      if (self$leg_id <= self$best_of) {

        next_starting_player <- self$leg$player_start + 1
        next_starting_player <-
          dplyr::if_else(next_starting_player > length(self$player_names),
    ...
    scoreboard = function() {
      ...
      join_tib <- tibble::tibble(winner = self$player_names)

      self$game_hist %>%
        dplyr::filter(!is.na(winner)) %>%
        dplyr::count(winner, sort = TRUE) %>%
        dplyr::full_join(join_tib, by = "winner") %>%
        tidyr::replace_na(list(n = 0))
    },

    scoreboard_dt = function() {
      self$scoreboard() %>%
        dplyr::rename("Player" = "winner", "Legs won" = "n") %>%
        DT::datatable(rownames = FALSE, selection = "none", options = list(
          paging = FALSE,
          info = FALSE,
          searching = FALSE,
          ordering = FALSE
